<template>
  <div id="kinetic-app">
    <nav v-if="isLoggedIn" class="nav">
      <router-link v-if="isAdmin" to="/wallet" class="nav-link">Wallet</router-link>
      <router-link v-if="isClient" to="/shop" class="nav-link">Boutique</router-link>
...
const isAdmin = computed(() => auth.user?.roles?.includes('ROLE_ADMIN') || isSuperAdmin.value)
const isClient = computed(() => auth.user?.roles?.includes('ROLE_CLIENT') && !isAdmin.value)
...
.nav-user { margin-left: auto; font-size: .85rem; color: #6b7280; }
.btn-logout { padding: .5rem 1rem; border: none; border-radius: 6px; background: #ef4444; color: #fff; cursor: pointer; }
